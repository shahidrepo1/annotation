...
export const AdAudioFolderModal = ({
  onSave,
  onClose,
  adData,
  selectedFiles,
    ...
    <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50 z-50">
      <div className="p-8 space-y-4 bg-white w-[900px] min-h-[450px] h-fit px-6 rounded-xl mx-auto shadow-lg border border-indigo-600 overflow-auto">
        ...
        <div className="mt-4 overflow-y-auto max-h-64 space-y-4 pr-1">
          {adData.chunks.map((audio) => (
            <div
              key={audio.id}
              className="flex items-center justify-between gap-4 bg-gray-50 border border-gray-200 p-4 rounded-lg shadow-sm"
            >
              <div className="flex items-center">
                <input
                  type="checkbox"
                  className="h-3 w-3 cursor-pointer accent-indigo-600"
                  checked={!!selectedFiles[audio.id]}
                  onChange={(e) => {
              ...
              <div className="w-1/3 text-center">
                <span className="inline-block text-sm font-medium bg-indigo-100 text-indigo-800 px-4 py-2 rounded-md transition duration-200">
                  {imageLabels[audio.id] || adData.label || "No label"}
                </span>
              </div>

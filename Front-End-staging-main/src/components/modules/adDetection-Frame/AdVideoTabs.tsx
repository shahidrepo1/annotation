...
const AdTabs = () => {
  ...
  const { data, isLoading } = useGetAdSegments();
  ...
  return (
    <div className="w-full p-4">
      ...
      <div className="mt-4 p-4 bg-white-100 rounded shadow">
        {activeTab === "ViewAll" && data && <AdViewAll data={data} />}
        {activeTab === "TrainedData" && data && <AdTrainedData data={data} />}
        {activeTab === "UntrainedData" && data && (
          <AdUntrainedData data={data} />
        )}
      </div>
    </div>
  );

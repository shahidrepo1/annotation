...
import useGetSentimentTrainedData from "../../../api/useGetSentimentTrainedData";
import { Model } from "../../../api/useSentimentTrainedData.types";

type VersionListProps = {
  onSelectVersion: (dailyVersion: Model | null) => void;
};

export const SentimentVersions = ({ onSelectVersion }: VersionListProps) => {
  const [searchTerm, setSearchTerm] = useState("");
  const [selectedVersionId, setSelectedVersionId] = useState<number | null>(
    null
  );
  const { data, isPending, isError } = useGetSentimentTrainedData();

  const filteredData: Array<Model> =
    Array.isArray(data?.results) && data.results.length > 0
      ? data.results.filter((version) =>

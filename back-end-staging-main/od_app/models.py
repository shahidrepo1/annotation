...
class MediaFile(models.Model):
    ...
    media_file = models.FileField(upload_to='od_media/',max_length=225)  # Store media files in 'od_media/' directory
...
class ProcessedImage(models.Model):
    ...
    confidence = models.FloatField(null=True, blank=True)  # Added for AI output
    start_time = models.CharField(max_length=20, null=True, blank=True)  # For video
    end_time = models.CharField(max_length=20, null=True, blank=True)    # For video
    coverage_time = models.FloatField(null=True, blank=True)             # For video
...
class ODTrainingVersions(models.Model):
...
class ODTrainingProgress(models.Model):
    STATUS_CHOICES = [
        ('training', 'Training'),
        ('completed', 'Completed'),
        ('failed', 'Failed'),
    ...
    user = models.ForeignKey(settings.AUTH_USER_MODEL,on_delete=models.CASCADE,related_name='od_trainings')
    module_name = models.CharField(max_length=255, null=True, blank=True)
    model_name = models.CharField(max_length=255)
    release_version = models.ForeignKey(ODTrainingVersions,on_delete=models.CASCADE,null=True,blank=True)

...
import useGetFrTrainedData from "../../../api/useGetFrTrainedData";
import { ModelType } from "../../../api/useGetFrTrainedData.types";

type VersionListProps = {
  onSelectVersion: (dailyVersion: ModelType | null) => void;
};

export const FrVersion = ({ onSelectVersion }: VersionListProps) => {
  const [searchTerm, setSearchTerm] = useState("");
  const { data, isPending, isError } = useGetFrTrainedData();
  ...
  return (
    <aside className="h-[86vh] text-indigo-400 p-4 overflow-auto w-64 border border-indigo-500 rounded-md">
      <ul className="space-y-2">
        ...
        {!isPending &&
          !isError &&
          filteredData.map((version: ModelType) => (
            <li
              key={version.id}

...
import useGetOcrTrainedData from "../../../api/useGetOCRUrduMediaTrainedData";

type Props = {
  onSelectVersion: (dailyVersion: TrainingModel | null) => void;
};

export const OCRMediaVersions = ({ onSelectVersion }: Props) => {
  const [searchTerm, setSearchTerm] = useState("");
  const [selectedVersionId, setSelectedVersionId] = useState<number | null>(
    null
  );

  const { data, isPending, isError } = useGetOcrTrainedData();

  const filteredData =
    Array.isArray(data) && data.length > 0
      ? data.filter((version: TrainingModel) =>
  ...
  return (
    <aside className="h-[86vh] text-indigo-400 p-4 overflow-auto w-64 border border-indigo-500 rounded-md">
      <ul className="space-y-2">
        ...
        {!isPending &&
          !isError &&
          filteredData.map((version: TrainingModel) => (
            <li
              key={version.id}

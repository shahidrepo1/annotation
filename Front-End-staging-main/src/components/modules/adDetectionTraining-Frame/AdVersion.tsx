...
import useGetAdTrainedData from "../../../api/useGetAdTrainedData";
import { Models } from "../../../api/useGetAdTrainedData.types";

type VersionListProps = {
  onSelectVersion: (dailyVersion: Models | null) => void;
};

export const AdVersion = ({ onSelectVersion }: VersionListProps) => {
  const [searchTerm, setSearchTerm] = useState("");
  const [selectedVersionId, setSelectedVersionId] = useState<number | null>(
    null
  );
  const { data, isPending, isError } = useGetAdTrainedData();

  const filteredData =
    Array.isArray(data) && data.length > 0
      ? data.filter((version: Models) =>
  ...
  return (
    <aside className="h-[86vh] text-indigo-400 p-4 overflow-auto w-64 border border-indigo-500 rounded-md">
      <ul className="space-y-2">
        ...
        {!isPending &&
          !isError &&
          filteredData.map((version: Models) => (
            <li
              key={version.id}

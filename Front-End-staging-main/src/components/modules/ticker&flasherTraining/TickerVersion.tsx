...
import useGetTfTrainedData from "../../../api/useGetTfTrainedData";
import { ModelInfoType } from "../../../api/useTfTrainedData.types";

type VersionListProps = {
  onSelectVersion: (dailyVersion: ModelInfoType | null) => void;
};

export const TickerVersion = ({ onSelectVersion }: VersionListProps) => {
  const [searchTerm, setSearchTerm] = useState("");
  const [selectedVersionId, setSelectedVersionId] = useState<number | null>(
    null
  );

  const { data, isPending, isError } = useGetTfTrainedData();

  const filteredData =
    Array.isArray(data) && data.length > 0
      ? data.filter((version: ModelInfoType) =>
  ...
  return (
    <aside className="h-[86vh] text-indigo-400 p-4 overflow-auto w-64 border border-indigo-500 rounded-md">
      <ul className="space-y-2">
        ...
        {!isPending &&
          !isError &&
          filteredData.map((version: ModelInfoType) => (
            <li
              key={version.id}

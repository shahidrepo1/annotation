...
# from .authentication import APIKeyAuthentication
from .process_image import extract_video_frame_at_timestamp, get_od_response, save_media_file
from .serializers import KnownLabelSerializer, ProcessedImageSerializer
from .utils import  create_od_training_progress, create_od_version, generate_pascal_voc_xml, group_by_label
from .models import KnownLabel, ProcessedImage, ODTrainingProgress
...
class ProcessMediaView(APIView):
    ...
    def post(self, request):
        try:
            ...
            od_response = get_od_response(file_path)
            print('-------this is od AI response', od_response)
            if od_response.get('status_code') != 200:
                return Response({"error": "od API error", "details": od_response}, status=500)
            
        
            print('--------OD AI processing started')
            response = od_response.get('response', [])
            predictions = response.get('detections') or response
            
 
            print('-------this od AI response', predictions)
            if not isinstance(predictions, list):
                return Response({"error": "Invalid od predictions format."}, status=500)

            saved_items = []
            for idx, pred in enumerate(predictions):
                # For backward compatibility, support both 'label' and 'object_name'
                label = pred.get('object_name', 'Unknown')
                coordinates = pred.get('coordinates', {})
                confidence = pred.get('confidence', None)
                start_time = pred.get('start_time') or pred.get('start', None)
                end_time = pred.get('end_time') or pred.get('end', None)
                coverage_time = pred.get('coverage_time', None)

                try:
                    label_obj = KnownLabel.objects.get(label_name=label)
                except KnownLabel.DoesNotExist:
                    label_obj,_ = KnownLabel.objects.get_or_create(label_name=label,id=0)  # Skip this prediction
                is_edited = label_obj.label_name == "Unknown"
                # Extract frame from video based on start_time if applicable
                if media_type == 'video':
                    # Use 0 if start_time is None or not convertible
                    try:
                        print("start_time: ",start_time , " end_time: ",end_time)
                        start_sec = self.time_str_to_seconds(start_time)
                        end_sec = self.time_str_to_seconds(end_time)
                        if start_sec and end_sec:
                            frame_time = (start_sec + end_sec) / 2
                        elif start_sec:
                        ...
                        frame_time = 0
                    frame_name = extract_video_frame_at_timestamp(file_path, frame_time)
                else:
...
class UpdateProcessedImageView(APIView):
    """
    Flexible update view for od processed images.
...
class ZipAndCreateODTrainingProgressAPI(APIView):
    """
    Creates an OD training version, zips ProcessedImages with Pascal VOC format, and sends the zip to AI API.
    """
    permission_classes = [IsAuthenticated]
    ...
    def post(self, request):
        try:
            ...
            zip_output_dir = os.path.join(settings.MEDIA_ROOT, "od_training_zips")
            os.makedirs(zip_output_dir, exist_ok=True)
            zip_path = os.path.join(zip_output_dir, f"{module_name}_od_train_data.zip")
            ...
            # Create OD version and training progress
            label_ids = list(
                images_to_include.filter(label__isnull=False)
                .values_list("label_id", flat=True)
                .distinct()
            )
            # You need to implement these two functions for OD, similar to TF
            version = create_od_version(module_name, label_ids, ids_for_version_creation)
            training_id = create_od_training_progress(user=user, module_name=module_name, version=version)

            # Zip images and Pascal VOC XMLs
            with zipfile.ZipFile(zip_path, 'w') as zipf:
                for image_obj in images_to_include:
                    try:
                        possible_paths = [
                            os.path.join(settings.MEDIA_ROOT, image_obj.processed_image),
                            os.path.join(settings.MEDIA_ROOT, 'od_media', image_obj.processed_image),
                            os.path.join(settings.MEDIA_ROOT, 'od_media', 'images', image_obj.processed_image)
                        ]
                        ...
                        continue
            threading.Thread(
                          target=send_zip_to_od_training_api,
...
# Immediately return a response to the user
            ...
            "zip_path": f"{settings.MEDIA_URL}od_training_zips/{os.path.basename(zip_path)}",
...
def send_zip_to_od_training_api(training_data_zip_path, training_id, training_data_folder):
    """Sends the zip file to the OD AI API and cleans up afterward. Returns success or failure gracefully."""
    try:
        with open(training_data_zip_path, 'rb') as f:
            ...
            ai_api_url = f"{settings.OD_TRAIN_API_URL}/train-objects/?training_id={training_id}"
            try:
                response = requests.post(ai_api_url, files=files)
                print(f"====----=====--  ***Response from OD AI API: {response.status_code} - {response.text}")
                if response.status_code == 200:
                    return {
                        "status": True,
                        "message": "OD AI training API call successful.",
                        "response_text": response.text
                    }
                else:
                    return {
                        "status": False,
                        "message": f"OD AI training API returned status {response.status_code}.",
                        "response_text": response.text
                    }
                ...
                return {
                    "status": False,
                    "message": f"Could not connect to OD training API: {str(e)}"
                }
...
class GetODTrainingProgressAPI(APIView):
    """
    API to fetch OD training progress records of the logged-in user,
    ...
    def get_processed_images(self, version):
        """
        Fetch and group ProcessedImages by label for a given ODTrainingVersion.
        Only images listed in version.version_images_id are considered 'trained'.
    ...
    def get(self, request):
        ...
        try:
            ...
            all_progress = ODTrainingProgress.objects.filter(
                user=request.user,
            ...
            print(f"Error in GetODTrainingProgressAPI: {e}")
            return Response({"error": str(e)}, status=500)
        

class UpdateODTrainingProgressAPI(APIView):
    """
    ...
    def post(self, request):
        ...
        try:
            training_progress = ODTrainingProgress.objects.get(id=training_progress_id)
        ...
        except ODTrainingProgress.DoesNotExist:

...
import useGetLogoTrainedData from "../../../api/useGetLogoTrainedData";
import { LogoModel } from "../../../api/useLogoTrainedData.types";

type VersionListProps = {
  onSelectVersion: (dailyVersion: LogoModel | null) => void;
};

export const LDVersion = ({ onSelectVersion }: VersionListProps) => {
  const [searchTerm, setSearchTerm] = useState("");
  const [selectedVersionId, setSelectedVersionId] = useState<number | null>(
    null
  );
  const { data, isPending, isError } = useGetLogoTrainedData();
  const filteredData =
    Array.isArray(data) && data.length > 0
      ? data.filter((version: LogoModel) =>
  ...
  return (
    <aside className="h-[86vh] text-indigo-400 p-4 overflow-auto w-64 border border-indigo-500 rounded-md">
      <ul className="space-y-2">
        ...
        {!isPending &&
          !isError &&
          filteredData.map((version: LogoModel) => (
            <li
              key={version.id}
